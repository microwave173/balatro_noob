...
class V13Runner:
    ...
    def run_game(self, deck: str = "RED", stake: str = "WHITE") -> Dict[str, Any]:
        ...
        for step in range(1, self.max_steps + 1):
            st = state.get("state")
            if self.verbose:
                print(f"[v1_3 step {step}] state={st} ante={state.get('ante_num')} round={state.get('round_num')} money={state.get('money')}")
            ...
            try:
                ...
                if st == "SELECTING_HAND":
                    ...
                    if cur_key != blind_plan_key:
                        plan_obs = self._obs(state, "PLAY", run_plan, "")
                    ...
                    phase_context = _merge_plan(blind_plan, action_feedback)
                    obs = self._obs(state, "PLAY", run_plan, phase_context)
                ...
                if st == "SHOP":
                    ...
                    obs = self._obs(state, "SHOP", run_plan, _merge_plan(shop_plan, action_feedback), recent_play_results=recent_play_results)

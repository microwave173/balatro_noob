...
from v1_4.core.deepseek_policy import _extract_json, _load_reference_guide
from v1_4.core.prompts import GAME_PRIMER, MASTER_IDENTITY, PLAY_DECISION_SCHEMA, STRATEGY_TIPS
...
def main() -> None:
    ...
    if not api_key:
        ...
        return
    runs = sorted(Path("v1_4/out_probe2/runs").glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    if not runs:
        print("[fatal] no v1_4/out_probe2 run found")
        return

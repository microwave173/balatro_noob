...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Balatro agent v1.4: observation-first DeepSeek player")
    ...
    p.add_argument("--state-file", default=os.getenv("V14_STATE_FILE", os.getenv("V13_STATE_FILE", "")))
...
def main() -> None:
    ...
    rpc_state_updater = _make_rpc_state_updater(args.state_file, args.provider, args.model, args.think, args.reasoning_effort, args.thinking_budget, args.decision_format)
    ...
    print(f"health={health.get('status')} version=v1_4 provider={args.provider} model={args.model} games={args.games}")
    if args.state_file:

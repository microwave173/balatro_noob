...
def render_compact_observation(
    ...
    *,
    ...
    obs["skills"] = _skill_items(relevant_skills)
    obs["rules"] = long_term_context.get("rules") or []
    obs["mistakes"] = long_term_context.get("mistakes") or []
...
def render_observation_text(obs: Dict[str, Any]) -> str:
    ...
    lines.append(f"- Learned skills relevant now: {_list_text(obs.get('skills'), 'none')}.")
    lines.append(f"- Long-term rules from reflection: {_list_text(obs.get('rules'), 'none')}.")
    lines.append(f"- Mistakes to avoid from reflection: {_list_text(obs.get('mistakes'), 'none')}.")
...
def _skills_line(skills: List[Dict[str, Any]]) -> str:
    if not skills:
        return "SKILLS: none"
    parts = []
    for s in skills[:8]:
        sid = _clean(s.get("id")) or "skill"
        conf = s.get("confidence", 0)
        pr = s.get("runtime_priority", s.get("priority", 0))
        text = _clean(s.get("policy_text")) or _clean(s.get("rule")) or ""
        parts.append(f"{sid},priority={pr:.2f},confidence={float(conf):.2f},{text}")
    return "SKILLS: " + " ; ".join(parts)


def _skill_items(skills: List[Dict[str, Any]]) -> List[str]:
    parts = []
    for s in skills[:8]:
        sid = _clean(s.get("id")) or "skill"
        conf = s.get("confidence", 0)
...
def _play_option_items(state: Dict[str, Any]) -> List[str]:
    ...
    for item in cards:
        ...
        by_suit.setdefault(str(value.get("suit") or ""), []).append(item)
    ...
    return [
        f"{opt['name']}: cards={opt['cards']}" + (f", note={opt['note']}" if opt.get("note") else "")
